...
class ExecutionPublisherTest(test_case_utils.TfxTest, parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      ...
      dict(
          ...
          invalid_uri='/my/original_uri/0/0'),
  ...
  def testPublishSuccessExecutionFailInvalidUri(self, invalid_uri):
    ...
    with metadata.Metadata(connection_config=self._connection_config) as m:
      ...
      execution_id = execution_publish_utils.register_execution(
          m, self._execution_type, contexts).id
      executor_output = execution_result_pb2.ExecutorOutput()
      new_example_1 = executor_output.output_artifacts[
          'examples'].artifacts.add()
      new_example_1.uri = invalid_uri
      new_example_2 = executor_output.output_artifacts[
          'examples'].artifacts.add()
      new_example_2.uri = '/my/original_uri/1'

      with self.assertRaisesRegex(
          RuntimeError,
          'When there are multiple artifacts to publish, the URI of them '
          'should be a direct sub-dir of the URI of the system generated '
          'artifact.'):
        execution_publish_utils.publish_succeeded_execution(
            m, execution_id, contexts, output_dict, executor_output)
